...
export class updateEntity1ById {
    ...
    this.app['post'](
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          bh = await this.sd_QFJKFc4UMyBNArdR(bh);
          //appendnew_next_sd_vxIXRFPAR04oNvcR
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_vxIXRFPAR04oNvcR');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
  ...
  async sd_QFJKFc4UMyBNArdR(bh) {
    try {
      this.sdService.addDMPropertiesToBh(bh, bh.web.req, 'entity_1');
      bh = await this.sd_FDbjNXkSFwPpoKmy(bh);
      //appendnew_next_sd_QFJKFc4UMyBNArdR
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_QFJKFc4UMyBNArdR');
    }
  }
