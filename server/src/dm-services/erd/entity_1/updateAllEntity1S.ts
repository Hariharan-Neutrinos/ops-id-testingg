...
export class updateAllEntity1S {
    ...
    this.app['post'](
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          bh = await this.sd_SetsPmmOdiVls6ni(bh);
          //appendnew_next_sd_KqqO4PKnqcLkPlj4
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_KqqO4PKnqcLkPlj4');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
  ...
  async sd_SetsPmmOdiVls6ni(bh) {
    try {
      this.sdService.addDMPropertiesToBh(bh, bh.web.req, 'entity_1');
      bh = await this.sd_8R9kDyRFeBfyGmoi(bh);
      //appendnew_next_sd_SetsPmmOdiVls6ni
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_SetsPmmOdiVls6ni');
    }
  }
