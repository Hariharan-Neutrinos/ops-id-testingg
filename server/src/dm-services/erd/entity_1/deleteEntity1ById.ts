...
export class deleteEntity1ById {
    ...
    this.app['post'](
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          bh = await this.sd_4hDlzY2pmHOfatMS(bh);
          //appendnew_next_sd_hLZbTm14lxMIqKP8
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_hLZbTm14lxMIqKP8');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
  ...
  async sd_4hDlzY2pmHOfatMS(bh) {
    try {
      this.sdService.addDMPropertiesToBh(bh, bh.web.req, 'entity_1');
      bh = await this.sd_mrHRC4rbzssWUUs6(bh);
      //appendnew_next_sd_4hDlzY2pmHOfatMS
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_4hDlzY2pmHOfatMS');
    }
  }

  async sd_mrHRC4rbzssWUUs6(bh) {
    try {
      ...
      //appendnew_next_sd_mrHRC4rbzssWUUs6
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_mrHRC4rbzssWUUs6');
    }
  }
  ...
  async sd_IpUaU51q741upXgr(bh) {
    try {
      ...
      return await this.errorHandler(bh, e, 'sd_tSTtGMMdYPsuIx3L');
    }
  }
