...
export class deleteAllEntity1S {
    ...
    this.app['post'](
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          bh = await this.sd_vexhlJcgNUzQdRJC(bh);
          //appendnew_next_sd_S9dGbIhHJWJ2m0xZ
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_S9dGbIhHJWJ2m0xZ');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
  ...
  async sd_vexhlJcgNUzQdRJC(bh) {
    try {
      this.sdService.addDMPropertiesToBh(bh, bh.web.req, 'entity_1');
      bh = await this.sd_GEdAyBz0T2PD5Hj2(bh);
      //appendnew_next_sd_vexhlJcgNUzQdRJC
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_vexhlJcgNUzQdRJC');
    }
  }

  async sd_GEdAyBz0T2PD5Hj2(bh) {
    try {
      const dmUtilsInst = new dmUtils('sd_MsveiTdT4FSAQYZr');
      bh.result = await dmUtilsInst.delete('_EN_5xopnlx24l', bh.filter);

      bh = await this.sd_IpUaU51q741upXgr(bh);
      //appendnew_next_sd_GEdAyBz0T2PD5Hj2
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_GEdAyBz0T2PD5Hj2');
    }
  }
  ...
  async sd_IpUaU51q741upXgr(bh) {
    try {
      ...
      return await this.errorHandler(bh, e, 'sd_y5P6ZUORtRiuj3eC');
    }
  }
