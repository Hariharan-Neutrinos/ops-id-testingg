...
export class createEntity1 {
    ...
    this.app['post'](
      ...
      async (req, res, next) => {
        ...
        try {
          ...
          //appendnew_next_sd_0XRNhfkIgeNkzsRY
        } catch (e) {
          return await this.errorHandler(bh, e, 'sd_0XRNhfkIgeNkzsRY');
        }
      },
      this.sdService.getMiddlesWaresBySequenceId(
        null,
  ...
  async sd_0grguYfj00JSLkeH(bh) {
    try {
      ...
      bh = await this.sd_v3BNreQuumFcKyr0(bh);
  ...
  async sd_v3BNreQuumFcKyr0(bh) {
    try {
      ...
      //appendnew_next_sd_v3BNreQuumFcKyr0
      return bh;
    } catch (e) {
      return await this.errorHandler(bh, e, 'sd_v3BNreQuumFcKyr0');
    }
  }
